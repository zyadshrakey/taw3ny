...
function Login() {
    ...
    async function sendLoginDataToApi(){
        try{
            let {data}= await axios.post(`https://wezaa.runasp.net/Auth/login`,user)
            console.log(data)
            if(!data.error){
    ...
    return <>
     <div className='d-flex flex-md-row flex-column justify-content-between'>
        ...
        <form onSubmit={submitLoginData} className='pt-3'>
            ...
            <div className='input-contaier position-relative py-2'>
                ...
                <input type='email' onChange={getUserData} name='email' placeholder='البريد الإلكترونى'  style={{paddingRight:'40px',border: '1px solid #777777',height:'48px',width:'503px', borderRadius:'16px'}} dir='rtl'/>
            </div>
            <div className='input-contaier position-relative py-2'>
                <i className="fas fa-key" style={{ position:'absolute',right:'20px', bottom:'25px'}}></i>
                <input type='password' onChange={getUserData} name='password' placeholder='كلمة المرور'  style={{paddingRight:'40px',border: '1px solid #777777',height:'48px',width:'503px', borderRadius:'16px'}} dir='rtl'/>
            </div>
                <button className='btn w-100 mt-2' style={{backgroundColor:'#214D97', color:'white', borderRadius:'24px', height:'48px'}}>
                { loading === true?<i className='fas fa-spinner fs-spin'></i>:' تسجيل الدخولِ'}
                    </button>
            
            
        </form>
        <Link className='pt-2' style={{textDecoration:'none', color:'black'}}>
        <p style={{textAlign:'left'}}>هل نسيت كلمة المرور؟</p></Link>

        <div className='line-text'>
            <p>أو قم بتسجيل الدخول بواسطة</p>
        </div>
        <div className='p-3 d-flex flex-md-row flex-cloumn align-items-center justify-content-center' style={{gap:'16px'}}>
            <div>
