...
function VolunteerRequestes({ userData }) {
  ...
  async function volunteerRequestApi() {
    try {
      ...
      let { data } = await axios.get(
        `https://wezaa.runasp.net/VolunteerApplications`,
        {
          headers: {
  ...
  const acceptedBack = async (id) => {
    ...
    try {
      await axios.put(
        `https://wezaa.runasp.net/VolunteerApplications/accept/${id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  const rejectBack = async (id) => {
    ...
    try {
      await axios.put(
        `https://wezaa.runasp.net/VolunteerApplications/reject/${id}`,
        {},
        {
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  return (
    <div className="container">
      <div className="title" dir="rtl">
        <h1
          className="py-2"
          style={{ fontWeight: "400", fontSize: "40px", lineHeight: "56px" }}
        >
          طلبات التطوع
        </h1>
      </div>
      {loading ? (
        <div className="d-flex justify-content-center align-items center">
          <i
            style={{ color: "#214D97", fontSize: "25px" }}
