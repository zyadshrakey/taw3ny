...
function UserRegister() {
    ...
    async function sendRegisterDataToApi(){
        try{
            let {data}= await axios.post(`https://wezaa.runasp.net/Auth/register-volunteer`,user)
            console.log(data)
            if(!data.error){
                navigate('login')
    ...
    return <>
     ...
     <div className='d-flex flex-md-row flex-column justify-content-between'>
            ...
            <div className="flex-column" style={{textAlign:'center', margin:'auto'}}>

            {error.length>0?
            <div className='alert alert-danger my-2'>{error}</div>:''}
                <p className="h1" style={{fontSize:'32px', fontWeight:'500', lineHeight:'38.73px', marginBottom:'32px'}}>مرحباً بك!
                    <br/>
                    قم بإنشاء حسابك
                </p>
                <div className="btnType d-flex justify-content-evenly align-items-center" style={{gap:'25px'}}>
                    <div>
                        ...
                        <button className="btn companyBtn" onClick={()=>handelClick(false)} style={isClicked === false ? buttonStyles.clicked : buttonStyles.default}>مؤسسة</button></Link>
                    </div>
                    <div>
                        <Link to="/" style={{ textDecoration: 'none', color:'white'}}>
                        <button className="btn userBtn" onClick={()=>handelClick(true)} style={isClicked === true ? buttonStyles.clicked : buttonStyles.default}>مستخدم</button></Link>
                    </div>
        ...
        <form onSubmit={submitRegisterData} className='pt-3'>
            <div className='input-contaier position-relative py-2'>
                ...
                <input type='text' onChange={getUserData} name='displayName' placeholder='اسم المستخدم'  className=' w-100' style={{paddingRight:'40px',border: '1px solid #777777',height:'48px', borderRadius:'16px'}} dir='rtl'/>
            </div>
            <div className='input-contaier position-relative py-2'>
            ...
            <button className='btn w-100 mt-2' style={{backgroundColor:'#214D97', color:'white', borderRadius:'24px', height:'48px'}}>
                { loading === true?<i className='fas fa-spinner fs-spin'></i>:'إنشاء حساب'}
            </button>
        </form>
        ...
        <p className="w-100" dir="rtl">
            ...
            <div className='sideUser-img'>
                {/* <img src={`/signup2.jpeg`} alt="login" style={{height:'100vh', width:'550px'}}/> */}
            </div>
        </div>
    </>
}
